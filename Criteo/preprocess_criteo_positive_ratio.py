...
# throw away some positive examples
ratio = 0.10 # the target fraction of positive examples in the trimmed dataset

pos_ratio = np.sum(full_data['Label']) / full_data.shape[0]
print("pos_ratio: {}".format(pos_ratio))
assert ratio < pos_ratio
pos_keep_ratio = (1 - pos_ratio) * ratio / (1 - ratio) / pos_ratio

row_choices = [] # indicator of which rows in the original data frame to keep
for num in full_data['Label']:
    if num == 0 or random.random() < pos_keep_ratio:
...
train_data.to_csv(path_or_buf='data/new_train_pfrac{}_0.9.csv'.format(ratio), index=False)
test_data.to_csv(path_or_buf='data/new_test_pfrac{}_0.1.csv'.format(ratio), index=False)