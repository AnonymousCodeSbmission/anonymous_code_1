...
def update_all_laser_attack_leak_auc(laser_attack_leak_auc_dict, grad_list, y):
    for (key, grad) in zip(laser_attack_leak_auc_dict.keys(), grad_list):
        normalized_grad = tf.math.l2_normalize(grad, axis=1)
        ip = tf.linalg.matmul(normalized_grad, normalized_grad, transpose_b = True)
        # print("ip: {}".format(ip))
        res = tf.math.sign(ip)
        # print("res: {}".format(res))
        indicator = tf.math.reduce_sum(res, axis = 1)
        # print("indicator: {}".format(indicator))
        indicator = tf.reshape(indicator, shape=[tf.shape(grad)[0]])
        indicator = -1.0 * indicator
        auc = update_auc(y=y,
                       predicted_value=indicator,
                       m_auc=laser_attack_leak_auc_dict[key])
