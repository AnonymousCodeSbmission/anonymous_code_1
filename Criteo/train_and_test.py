...
from model import update_all_norm_leak_auc, update_all_hint_norm_attack_leak_auc, update_all_hint_inner_product_attack_leak_auc, print_all_leak_auc, reset_all_leak_auc, tf_summary_all_leak_auc, update_all_laser_attack_leak_auc
...
def train(model, train_set, test_set, loss_function, num_epochs, writer,
    ...
    laser_leak_auc_dict = model.leak_auc_dict(attack_method='laser_leak')
    ...
    for epoch in range(num_epochs):
        ...
        for (batch_idx, (X, y)) in enumerate(train_set, 1):
            ...
            update_all_norm_leak_auc(norm_leak_auc_dict, activation_grad_list, y)
            update_all_laser_attack_leak_auc(laser_leak_auc_dict, activation_grad_list, y)
            ...
            if batch_idx == 1 or batch_idx % period == 0:
                ...
                # print_all_leak_auc(leak_auc_dict=laser_leak_auc_dict)
                if num_hints is not None:
                    for n_hint in num_hints:
                ...
                with writer.as_default():
                    ...
                    tf_summary_all_leak_auc(laser_leak_auc_dict, step=shared_var.counter)
                        ...
                        if epoch in [0, 1, 2]:
                            for name, item in info.items():
                                if name in ['pos_grad_norm', 'neg_grad_norm']:
                                    np.save(file=os.path.join(shared_var.logdir, 'epoch' + str(epoch) + '_' + layer_name + '_' + name + '_' + 'batch' + str(batch_idx)),
                                            arr=item.numpy())
                        '''
                ...
                reset_all_leak_auc(laser_leak_auc_dict)
                if num_hints is not None:
                    for n_hint in num_hints:
        ...
        e_e = datetime.datetime.now()
        print("epoch {} used: {}".format(epoch, e_e - e_s))
        print()

    print("best test auc: {:.4f}".format(best_test_auc))
    print("best epoch: ", best_epoch)
    print("total used: {}".format(datetime.datetime.now() - t_s))
